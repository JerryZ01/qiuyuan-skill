...
核心设计：**Phase 1.9 跨维度交叉阅读**（主 session 在 8 路报告完成后统一读取并生成 `09-cross-matrix.md`）。寻找印证、矛盾、涌现，发现球员多条因果链，而非孤立事实。
...
│   ├── merge_research.py             # 调研合并 + Phase 2 检查点生成
